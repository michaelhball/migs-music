...
class LibraryRepository(
    ...
    fun observeSongsInFolder(folderPath: String): Flow<List<SongEntity>> =
        songDao.observeSongsInFolder(folderPath)
    ...
    fun observeDirectSongsIn(parentPath: String): Flow<List<SongEntity>> =
        songDao.observeSongsInFolder(parentPath)

    /** All songs under [parentPath] including subfolders. Used for "Play Folder" at any level. */
    fun observeSongsRecursivelyIn(parentPath: String): Flow<List<SongEntity>> =
        songDao.observeSongsRecursivelyIn(parentPath.trimEnd('/'))
    ...
    fun observeSongsByArtist(artist: String): Flow<List<SongEntity>> =
        songDao.observeSongsByArtist(artist)

    fun observeSongsByAlbum(albumKey: String): Flow<List<SongEntity>> =
        songDao.observeSongsByAlbum(albumKey)

    private fun buildSubfolders(all: List<SongEntity>, parentPath: String): List<FolderSummary> {
    ...
    suspend fun scanDevice(): Int = withContext(Dispatchers.IO) {
        ...
        val cursor = contentResolver.query(collection, projection, selection, null, sortOrder)
