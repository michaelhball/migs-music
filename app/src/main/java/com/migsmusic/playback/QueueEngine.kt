...
class QueueEngine(
    ...
    fun restore(restoredState: QueueState, restoredSeed: Long) {
        state = restoredState
    ...
    fun moveUpcoming(entryId: String, newIndex: Int): QueueState? {
        val current = state ?: return null
    ...
    fun skipToNext(): QueueState? {
        val current = state ?: return null
        val nextItem = current.nextItems.firstOrNull()
        ...
        val laterItems = if (current.nextItems.isEmpty()) {
            current.laterItems.dropWhile { it.entryId == nextItem.entryId }
        } else {
            current.laterItems
        }
        val remainingItems = if (current.nextItems.isEmpty() && current.laterItems.isEmpty()) {
    ...
    private fun buildShuffledOrder(songIds: List<Long>, startIndex: Int): List<Long> {
