...
private class FakePlaybackSnapshotDao : PlaybackSnapshotDao {
    ...
    override suspend fun upsertSnapshot(snapshot: PlaybackSnapshotEntity) { stored = snapshot }
}

class PlaybackSessionRepositoryTest {
    @Test
    fun saveAndLoadRoundTripsSongIdsAndPositionAndModes() = runBlocking {
        ...
        val allEntries = restored.queueState.history +
    ...
    @Test
    fun loadReturnsNullWhenNoSnapshotExists() = runBlocking {
        val repo = PlaybackSessionRepository(FakePlaybackSnapshotDao())
        assertNull(repo.load())
    }

    @Test
    fun loadReturnsNullWhenCurrentSongIdMissing() = runBlocking {
        val dao = FakePlaybackSnapshotDao()
        dao.stored = PlaybackSnapshotEntity(
            currentEntryId = null,
            currentSongId = null,
            currentPositionMs = 0,
            isPlaying = false,
    ...
    @Test
    fun saveWithEmptyQueueStateProducesEmptySerializedFields() = runBlocking {
        val dao = FakePlaybackSnapshotDao()
        val repo = PlaybackSessionRepository(dao)
        repo.save(
            queueState = null,
    ...
    @Test
    fun loadIgnoresMalformedSongIdTokens() = runBlocking {
        val dao = FakePlaybackSnapshotDao()
        dao.stored = PlaybackSnapshotEntity(
            currentEntryId = null,
            currentSongId = 13,
